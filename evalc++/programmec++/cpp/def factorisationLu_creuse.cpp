...
{   int N = matrix.size;
    ...
    for(int i = 2; i<N-1; i++)
    {
        ...
        L.diago_down[i-1]= (matrix.diago_down[i-1]) / U.diago[i-2];
        U.diago_up[i-1] = matrix.diago_up[i-1];
        U.diago[i-1] = matrix.diago[i-1] - L.diago_down[i-1] * U.diago_up[i-1];
    };
    
    L.diago[N-1]  = 1;
    U.diago_up[N-2] = matrix.diago_up[N-2] ;
    L.diago_down[N-2]= (matrix.diago_down[N-2]) / U.diago[N-2];

    U.diago[N-1] = matrix.diago[N-1] - L.diago_down[N-2] * U.diago_up[N-2];
